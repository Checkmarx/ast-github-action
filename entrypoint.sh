#!/bin/sh
/app/bin/cx scan create --project-type ${PROJECT_TYPE} -v --base-uri ${BASE_URI} --project-name ${PROJECT_NAME} -d ${SOURCE_DIRECTORY} $( (( -v ${FILTER} )) && printf %s '--filter ${FILTER}' ) $( (( -v ${CLIENT_ID} )) && printf %s '--client-id ${CLIENT_ID}' ) $( (( -v ${SECRET} )) && printf %s '--secret ${SECRET}' )
